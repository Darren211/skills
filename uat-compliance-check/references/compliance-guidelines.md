...
**CI 变量（GitLab：Settings → CI/CD → Variables）**：`AWS_ACCOUNT_ID`、`AWS_REGION`、`DOCKER_REGISTRY` 等由 CI 配置；登录密码使用 Secret Variable（Masked）。CI 内使用 `aws ecr get-login-password` 或变量，禁止在 YAML 中写死密码。

**Makefile**：`DOCKER_REGISTRY ?= $(DOCKER_REGISTRY)`，禁止硬编码 `registry.xxx.xyz`。

### 7.6 私钥安全处理规范

**安全审计要求**：RSA/ECDSA 等私钥不得通过配置文件路径明文传入；优先从环境变量或 Secrets Manager 读取，使用后尽量从内存中清除敏感字节。禁止在 `application.yml`/`config.toml` 中配置私钥路径或内容供生产使用。配置模板中仅占位说明，如 `# SIGNING_PRIVATE_KEY=`。
